...
// Build with -DUSE_MKL=1
#if USE_MKL
// The necessary #includes for MKL are in VerifyTransform.hpp.
void verify3d(fftx::box_t<3> a_domain,
              fftx::box_t<3> a_fdomain,
              int a_rounds,
              mklTransform3dType<std::complex<double>, std::complex<double>>& a_mddft,
              mklTransform3dType<std::complex<double>, std::complex<double>>& a_imddft,
              mklTransform3dType<double, std::complex<double>>& a_prdft,
              mklTransform3dType<std::complex<double>, double>& a_iprdft,
              int a_verbosity)
{
...

  {
    ...
    TransformFunction<3, std::complex<double>, std::complex<double>>
      fun(a_mddft, a_domain, a_domain, fullExtents, name, MKL_FORWARD);
...

  {
    ...
    std::cout << "***** test 3D IMDDFT on complex "
              << a_domain << std::endl;
    TransformFunction<3, std::complex<double>, std::complex<double>>
      fun(a_imddft, a_domain, a_domain, fullExtents, name, MKL_BACKWARD);
    VerifyTransform<3, std::complex<double>, std::complex<double>>
      (fun, a_rounds, a_verbosity);
  }

  {
    ...
    TransformFunction<3, double, std::complex<double>>
      fun(a_prdft, a_domain, a_fdomain, fullExtents, name, MKL_FORWARD);
...

  {
    ...
    TransformFunction<3, std::complex<double>, double>
      fun(a_iprdft, a_fdomain, a_domain, fullExtents, name, MKL_BACKWARD);
...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  verify3d(domain3, fdomain3, rounds,
           mddft3dMKL, imddft3dMKL,
           mdprdft3dMKL, imdprdft3dMKL,
           verbosity);
#endif
