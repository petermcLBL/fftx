...
// Build with -DUSE_FFTW3=1
#if USE_FFTW3
// The necessary #includes for FFTW3 are in VerifyTransform.hpp.
void verify3d(fftx::box_t<3> a_domain,
              fftx::box_t<3> a_fdomain,
              int a_rounds,
              fftwTransform3dType<std::complex<double>, std::complex<double>>& a_mddft,
              fftwTransform3dType<std::complex<double>, std::complex<double>>& a_imddft,
              fftwTransform3dType<double, std::complex<double>>& a_prdft,
              fftwTransform3dType<std::complex<double>, double>& a_iprdft,
              int a_verbosity)
{
...

  {
    ...
    TransformFunction<3, std::complex<double>, std::complex<double>>
      fun(a_mddft, a_domain, a_domain, fullExtents, name, FFTW_FORWARD);
...

  {
    ...
    std::cout << "***** test 3D IMDDFT on complex "
              << a_domain << std::endl;
    TransformFunction<3, std::complex<double>, std::complex<double>>
      fun(a_imddft, a_domain, a_domain, fullExtents, name, FFTW_BACKWARD);
    VerifyTransform<3, std::complex<double>, std::complex<double>>
      (fun, a_rounds, a_verbosity);
  }

  {
    ...
    TransformFunction<3, double, std::complex<double>>
      fun(a_prdft, a_domain, a_fdomain, fullExtents, name);
...

  {
    ...
    TransformFunction<3, std::complex<double>, double>
      fun(a_iprdft, a_fdomain, a_domain, fullExtents, name);
...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  verify3d(domain3, fdomain3, rounds,
           mddft3dFFTW, imddft3dFFTW,
           mdprdft3dFFTW, imdprdft3dFFTW,
           verbosity);
#endif
